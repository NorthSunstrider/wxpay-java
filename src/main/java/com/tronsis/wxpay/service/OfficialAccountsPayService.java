...
public class OfficialAccountsPayService {
	public OfficialAccountsPayRequest createPayment(String description, String orderSn, Integer totalPrice, String openid)
			throws Exception {
		String nonce_str = Util.getRandomString(8);
		String callbackUrl = WxpayConfig.UNIFIED_ORDER_CALLBACK_URL;
		// FIXME 在生产环境中使用以下第一行，并注释以下第二行
		// String ip = request.getRemoteHost();
		String ip = "59.40.98.35";
		...
		UnifiedOrderRequest unifiedOrderRequest = new UnifiedOrderRequest(WxpayConfig.APPID, WxpayConfig.MCH_ID,
				WxpayConfig.DEVICE_INFO_WEB, nonce_str, description, orderSn, totalPrice, ip, callbackUrl, WxpayConfig.TRADE_TYPE_JSAPI,
				orderSn);
		...
		if (unifiedOrderResponse != null && WxpayConfig.RETURN_CODE_SUCCESS.equals(unifiedOrderResponse.getResult_code())
